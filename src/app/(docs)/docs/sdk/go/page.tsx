...
export default function GoClientDocsPage() {
  const [copied, setCopied] = useState(false);
  const [copiedCode, setCopiedCode] = useState(false);
  const [isLoadingTerminal, setIsLoadingTerminal] = useState(false);
  const [terminalOutput, setTerminalOutput] = useState<string | null>(null);

  const installTemplate = `go get github.com/tobisamuel/chainpulse-go`;
...
func main() {
	...
	wallet, err := client.Wallets.Create(context.Background(), chainpulse.WalletParams{
		Chain:                 "bsc",
	...
	fmt.Printf("Wallet registered! ID: %s\\n", wallet.ID)
}`;
      ...
      <main style={{ overflowY: 'auto' }} className="docs-main-content">
        <section style={{ marginBottom: '32px' }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '14px', lineHeight: 1.6 }}>
            Integrate ChainPulse into your Go microservices using our official Go client library.
          </p>
        </section>
        ...
        <section style={{ marginBottom: '40px', paddingTop: '24px', borderTop: '1px solid var(--border-color)' }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '14px', lineHeight: 1.6, marginBottom: '16px' }}>
            Add the module to your go.mod file:
          </p>
      ...
      <aside style={{ overflowY: 'auto' }} className="docs-sandbox-sidebar">
        ...
        <div className="cyber-terminal">
          <div className="terminal-header">
            ...
            <div className="terminal-title">Go Integration Engine</div>
          </div>
          <div className="terminal-body" style={{ padding: '16px' }}>
            <p style={{ color: 'var(--text-muted)', fontSize: '12px', margin: '0 0 12px 0' }}>
              Run the Go compiler simulator to verify dependencies.
            </p>
            <button
              className="cyber-btn"
            ...
            >
              <Play size={12} /> {isLoadingTerminal ? 'Building...' : 'Run Integration'}
            </button>
